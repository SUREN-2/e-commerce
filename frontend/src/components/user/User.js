...
const User = () => {
  ...
  const handleLogout = () => {
    ...
    // Clear authToken and refreshToken from localStorage
    // localStorage.removeItem('authToken');
    // localStorage.removeItem('refreshToken');

    // // Clear refreshToken from HTTP headers
          ...
          <div className="flex-shrink-0 w-full lg:w-80 mr-7 lg:mr-10 xl:mr-10">
            <div className="bg-white p-4 sm:p-5 lg:p-8 rounded-md sticky top-32">
              ...
              <span className="p-2 flex  items-center rounded-md hover:bg-gray-50 w-full hover:text-emerald-600">
                ...
                <button className="inline-flex text-black items-center justify-between text-sm font-medium w-full hover:text-emerald-600" onClick={handleLogout}>
                  Logout
                </button>
              </span>
            </div>
